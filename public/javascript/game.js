...
const renderNewMessage = (message) => {
  const noChatOverlay = document.getElementById("noChatOverlay");
  ...
  noChatOverlay.classList.add("hidden");
};
...
const startBtnOnClick = (socket, startRoundBtn, gameId) => {
  if (!startRoundBtn.disabled) socket.emit("startRound", { gameId, roomId });
};

const onSocketUpdateGameHistory = (
  gameHistory,
  lastMove,
  createGameHistoryCard
) => {
